...
const Inventory = ({ id }: { id: any }) => {
  ...
  useEffect(() => {
    let itemFilter = itemSelector.filter((val: any) => val.key === id);
  ...
  const handleItemsInp = (e: any) => {
    let { name, value } = e.target;
    setInputValues({ ...inputValues, [name]: value });
  };

  // select product image
  const handleWatchImg = (event: any) => {
  ...
  function uploadPdfAndImageOnDB(
    toCheck: any,
    newFiles: any,
    oldFile: any,
    nameOf: any,
  ...
  const handleAddAndUpdateItem = (e: any) => {
    e.preventDefault();
    ...
    <div className="inventoryContainer">
      ...
      {active ? (
        <>
          ...
          <button onClick={disConnect} className="cus_btn">
            Disconnect
          </button>
        </>
      ) : (
